...
def replace_background_with_nearest_label(seg, gt):
    ...
    # Compute the distance transform for the non-background regions
    distances, nearest_indices = distance_transform_edt(foreground_mask == 0, return_indices=True)
    
    # Create a copy of 'seg' to modify
    seg_filled = seg.copy()
...
def dilate_line(image, line_mask, dilation_size=1):
    ...
    structuring_element = generate_binary_structure(3, 1)  # Reuse for dilation
    ...
    # Set the dilated line in the original image (link the components)
...
def link_components(binary_image, min_distance=15):
    ''' This code is used inside a loop that iterates over each class '''
    ...
    while num_features > 1:  # If one class have disconnected components, we will try to link them when possible..
        ...
        for pair in closest_pairs:
            ...
            # print(f"Closest pair: Component {comp1} and Component {comp2}")
            # print(f"Distance: {distance:.2f} | Point1: {point1} | Point2: {point2}\n")
            break


        if distance < 15: #min_distance:   #FIXME: # If the distance between the two found closest components is not that big, then we link them.. otherwise we skip
...
def final_check_wrongly_submitted(multiclass_image):
    ''' This function is used as a final check, to remove small disconnected components, or relabel them as the structure that is connected to them (to ensure continuity..hopefully!) '''
    new_mask = multiclass_image.copy()
    
    for i in np.unique(multiclass_image):
        binary_image = 1*(new_mask==i)  # Dealing with 1 class label per time
        labeled_image, num_features = label(binary_image)        
        ...
        while num_features > 1:  # In case even after trying to link disconnected components, there are stil some remaining, then we either remove them (if they are not connected to anything) or relabel them (if they have neighboring component, they take that label)
            ...
            # Check connectivity to other components using multiclass_image
            if smallest_component_coords.size > 0:
                ...
                if len(unique_labels) > 0:
                    # Assign the label of the first neighbor component
                    
                    new_label = unique_labels[0]
                    new_mask[labeled_image == smallest_component_idx] = new_label
                else:
                    # Delete the smallest disconnected component
    ...
    for i in np.unique(multiclass_image):
        ...
        while num_features > 1:  # In case even after trying to link disconnected components, there are stil some remaining, then we either remove them (if they are not connected to anything) or relabel them (if they have neighboring component, they take that label)
            ...
            if smallest_component_coords.size > 0:
                ...
                labeled_image, num_features = label(1*(new_mask==i)) 
            # break
