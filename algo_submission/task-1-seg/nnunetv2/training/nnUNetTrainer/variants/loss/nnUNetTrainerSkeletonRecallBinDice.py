...
from nnunetv2.training.loss.compound_losses import DC_SkelREC_CE_and_BinDC_loss
...
class nnUNetTrainerSkeletonRecallBinDice(nnUNetTrainer):
    ...
    def _build_loss(self):
        ...
        loss = DC_SkelREC_CE_and_BinDC_loss(soft_dice_kwargs={'batch_dice': self.configuration_manager.batch_dice,
                                                        'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, 
                                      soft_skelrec_kwargs={'batch_dice': self.configuration_manager.batch_dice,
                                                           'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, 
                                      ce_kwargs={}, weight_ce=0.8, weight_dice=0.8, weight_bdice=1, weight_srec=self.weight_srec, #FIXME: modified weights from 1 to 0.5
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # print('output.shape:', output[:,0,:,:,:].shape)  #FIXME: TODO: del later
            # print('output.min max:', output[:,0,:,:,:].min(), output[:,0,:,:,:].max())  #FIXME: TODO: del later
            # print('output[32,:,:]:', output[0,0,32,:,:])  #FIXME: TODO: del later
            # print('target[32,:,:]:', (1*(target>0))[0,:,32,:,:])  #FIXME: TODO: del later
