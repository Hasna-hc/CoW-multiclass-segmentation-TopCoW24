...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_and_FL_loss
...
from nnunetv2.utilities.sam import SAM, disable_bn, enable_bn, enable_running_stats, disable_running_stats
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.initial_lr = 1e-2 #1e-4  #FIXME: #1e-2
        self.weight_decay = 3e-5
        self.oversample_foreground_percent = 0.8  # FIXME: originally 0.33    #0.33  #FIXME 0.7
        self.num_iterations_per_epoch = 250
        self.num_val_iterations_per_epoch = 50
        self.num_epochs = 1000 #300 #1000  #FIXME:
        self.current_epoch = 0
        self.enable_deep_supervision = True
        self.num_patches = 32 #8  #FIXME: added this param to change the number of patches to extract per image
        self.use_focal_loss = False  # TODO: #FIXME: Adding the focal_loss option here (when True, FL+Dice is used)
        ...
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                   {'batch_dice': self.configuration_manager.batch_dice,
                                    'do_bg': True, 'smooth': 1e-5, 'ddp': self.is_ddp}, weight_ce=1, weight_dice=1,  #FIXME: Added weight_ce=1, weight_dice=0.1,
                                   use_ignore_label=self.label_manager.ignore_label is not None,
                                   dice_class=MemoryEfficientSoftDiceLoss)
        else:
            loss = DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,  #FIXME: modified weight_dice from 1 to 0.1
                                  ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)

        if self.use_focal_loss: #TODO: FIXME: Adding here the FocalLoss possibility through a variable
            self.print_to_log_file(f"Using FL+DC compound loss, with self.use_focal_loss: {self.use_focal_loss}")
            loss = DC_and_FL_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_focal=1, weight_dice=1,  #FIXME: modified weight_dice from 1 to 0.1
                                  ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
    ...
    def configure_optimizers(self):  #FIXME: this one is the original
        optimizer = torch.optim.SGD(self.network.parameters(), self.initial_lr, weight_decay=self.weight_decay,
                                    momentum=0.99, nesterov=True)
        lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        return optimizer, lr_scheduler
    
    # def configure_optimizers(self):  #FIXME: modified config to used SAM
    #     ### SGD Optimizer
    #     param_groups = [
    #         {'params': self.network.parameters(), 'weight_decay': self.weight_decay, 'lr': self.initial_lr},
    #         # {'params': self.loss.parameters(), 'weight_decay': 0, 'lr': self.initial_lr}
    #     ]
    #     base_optimizer = torch.optim.SGD
    #     # optimizer = SAM(self.network.parameters(), base_optimizer, momentum=0.99, nesterov=True)
    #     optimizer = SAM(param_groups, base_optimizer, momentum=0.99, nesterov=True)

    #     ### Adam Optimizer
    #     # base_optimizer = torch.optim.Adam
    #     # optimizer = SAM(self.network.parameters(), base_optimizer, lr=self.initial_lr, weight_decay=self.weight_decay)
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if not isfile(splits_file):
                ...
                splits = generate_crossval_split(all_keys_sorted, seed=12345, n_splits=5)  #FIXME:
    ...
    def get_dataloaders(self):
        ...
        if dim == 2:
            ...
            dl_tr = nnUNetDataLoader3D(dataset_tr, self.batch_size,
                                       ...
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       num_patches=self.num_patches,  #FIXME: added this parameter
                                       sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = nnUNetDataLoader3D(dataset_val, self.batch_size,
                                        ...
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
                                        num_patches=1,  #FIXME: added this parameter
                                        sampling_probabilities=None, pad_sides=None, transforms=val_transforms)
    ...
    @staticmethod  #FIXME: here are the training data augmentation parameters..  #TODO:
    def get_training_transforms(
        ...
        transforms.append(RandomTransform(  #FIXME: removed this one to check whether the LR simulation make it worse for pvs detection..
            SimulateLowResolutionTransform(
                scale=(0.35, 1),  #FIXME: originally (0.5, 1)
                synchronize_channels=False,
    ...
    def on_train_epoch_start(self):
        self.network.train()
        self.lr_scheduler.step(self.current_epoch)  #FIXME: 
        self.print_to_log_file('')
    ...
    def train_step(self, batch: dict) -> dict:  #FIXME: original code
        data = batch['data']
        target = batch['target']
    ...
    # def train_step(self, batch: dict) -> dict:  # FIXME: Added SAM here.. and cleaned around to remove autocast! ..
    #     data = batch['data']
    #     target = batch['target']
    ...
    #     assert not torch.isnan(data).any(), "Input contains NaNs"  #TODO:
    #     assert not torch.isinf(data).any(), "Input contains infinite values"  #TODO:
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):  #TODO: is there anything relevant here??
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output = self.network(data)
            del data
            l = self.loss(output, target)  #TODO: Add here the ROI mask..
        ...
        tp, fp, fn, _ = get_tp_fp_fn_tn(predicted_segmentation_onehot, target, axes=axes, mask=mask)  #TODO: maybe add ROI mask here too..?
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        self.logger.log('val_losses', loss_here, self.current_epoch)  #TODO: check whether it needs to be masked by the ROI..
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):  #FIXME: the validation and TTA with 'nnUNetPredictor'
        ...
        predictor = nnUNetPredictor(tile_step_size=0.5, use_gaussian=True, use_mirroring=True,  #FIXME: step size for SW was 0.5
